...
class channel(commands.Cog, description="Channel utils"):
    ...
    @app_commands.command(name="slowmod", description="Set Slowmod in current Channel")
    @app_commands.guild_only()
    @app_commands.guilds(785839283847954433)
    @app_commands.default_permissions(manage_messages=True)
    @app_commands.describe(timer="Slowmod Duration")
    async def slowmode(self, interaction:discord.Interaction, timer: str = '0'):

        timer = await convert_to_time(timer)
        cd = await calculate(timer)
        
        if cd > 21600:
            await interaction.response.send_message('Slowmode duration cannot be more than 6 hours', ephemeral=True)
        elif cd == 0:
    ...
    @app_commands.guilds(785839283847954433)
    @app_commands.default_permissions(manage_messages=True)
    @app_commands.describe(role="Enter role to lock channel for it")
    async def lock(self, interaction: discord.Interaction, role: discord.Role = None):

        channel = interaction.channel
        role = role if role else discord.utils.get(interaction.guild.roles, id=self.default_role)

        overwrite = channel.overwrites_for(role)
        overwrite.send_messages = False

        await channel.set_permissions(role, overwrite=overwrite)
        await interaction.response.send_message(f"Locked {channel.mention} for {role.mention}", hidden=True)
        embed = discord.Embed(
    ...
    @app_commands.guilds(785839283847954433)
    @app_commands.default_permissions(manage_messages=True)
    @app_commands.describe(role="Enter role to lock channel for it", state="Enter state to unlock channel for it")
    async def unlock(self, interaction: discord.Interaction, state: bool = True, role: discord.Role = None):
        ...
        await interaction.response.send_message(f"Unlocked {channel.mention} for {role.mention} with state `{state}`", hidden=True)
        await channel.set_permissions(role, overwrite=overwrite)
    ...
    @app_commands.command(name="hide", description="Hide current Channel")
    @app_commands.guild_only()
    @app_commands.guilds(785839283847954433)
    @app_commands.default_permissions(manage_messages=True)
    @app_commands.describe(role="Enter role to hide channel for it")
    async def hide(self, interaction: discord.Interaction, role: discord.Role = None):
        channel = interaction.channel
        role = role if role else discord.utils.get(interaction.guild.roles, id=self.default_role)
        overwrite = channel.overwrites_for(role)
        # overwrite.send_messages = False
    ...
    @app_commands.command(name="unhide", description="Unhide current Channel")
    @app_commands.guild_only()
    @app_commands.guilds(785839283847954433)
    @app_commands.default_permissions(manage_messages=True)
    @app_commands.describe(role1="Enter role to unhide channel for it", state="Enter state to unhide channel for it", role2="Enter role to unhide channel for it", role3="Enter state to unhide channel for it")
    async def unhide(self, interaction: discord.Interaction, state: bool = True, role1: discord.Role = None, role2: discord.Role = None, role3: discord.Role = None):
        channel = interaction.channel
        ...
        if role1 != None:
            if state == True:
                overwrite[role1] = discord.PermissionOverwrite(view_channel=True)
            elif state == False:
                overwrite[role1] = discord.PermissionOverwrite(view_channel=False)
            embedMention = embedMention + role1.mention + " "
        if role2 != None:
            if state == True:
                overwrite[role2] = discord.PermissionOverwrite(view_channel=True)
            elif state == False:
                overwrite[role2] = discord.PermissionOverwrite(view_channel=False)

            embedMention = embedMention + role2.mention + " "
        if role3 != None:
            if state == True:
                overwrite[role3] = discord.PermissionOverwrite(view_channel=True)
            elif state == False:
                overwrite[role3] = discord.PermissionOverwrite(view_channel=False)                
        ...
        await interaction.response.send_message(f"{embedMention} can view **{channel}** now", hidden=True)
        embed = discord.Embed(
            color=0x78AB46, description=f'{msg}')
        await channel.send(embed=embed)
    
    @app_commands.command(name="dankdown", description="Use this comand when dank is offline")
    @app_commands.guild_only()
    @app_commands.guilds(785839283847954433)
    @app_commands.default_permissions(administrator=True)
    async def dankdown(self, interaction:discord.Interaction):
        await interaction.response.send_message(f"Dank is offline", hidden=True)
        ...
        await interaction.edit_original_message(content=f"Dank is LockedUp")
    ...
    @app_commands.command(name="dankup", description="Use this commands when dank comes back online",)
    @app_commands.default_permissions(administrator=True)
    @app_commands.guild_only()
    @app_commands.guilds(785839283847954433)
    async def dankup(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Unlocking Dank Memer channels")
        async with interaction.typing():
            default_role = discord.utils.get(interaction.guild.roles, id=self.default_role)
            lv = discord.utils.get(interaction.guild.roles, id=811307500321505320)
        ...
        await interaction.edit_original_message(content=f"Dank is Unlocked")
 
    @app_commands.command(name="sync", description="sync channel permissions with category")
    ...
    async def sync(self, interaction: discord.Interaction):
        ...
        channel = interaction.channel
        await interaction.response.send_message(f" **{channel}** synced with **{channel.category.name}**", hidden=True)
        embed = discord.Embed(
