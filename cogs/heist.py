...
class dankHeist(commands.Cog,name="Dank Heist", description="Heist Manager"):
    ...
    #     embed = discord.Embed(
    #         color=0x78AB46, description=f' {self.bot.emojis_list["SuccessTick"]} | ViewLocked **{heist_channel}** for {everyone_role} & {default_role.mention}')
    #     await a_message.edit(embed=embed, allowed_mentions=am)
    ...
    #     await starter.add_roles(starter_role)
    #     await announcement_channel.send(embed=starter_embed, delete_after=45)
    #     await announcement_channel.send(f"{starter.mention} start the heist in {heist_channel.mention}", delete_after=45)
    ...
    #         dm = await self.bot.wait_for("message", check=lambda m: m.author.id == 270904126974590976 and m.content == "", timeout=9)
    #         if dm.embeds[0]:
    #             print(f"{dm.embeds[0]}")
    #             if  "They're trying to break into" in dm.embeds[0].description:
    #                 # break
    ...
    #                 unlock_embed = discord.Embed(
    #                     title=f"<a:tgk_run:832700446711611422>       **{'Channel unlocked'}**       <a:tgk_run:832700446711611422> ",
    #                     description=f"{req_role.mention if req_role != ctx.guild.default_role else req_role} can now view the channel!\n",
    #                     color=ctx.author.color,
    #                     timestamp=datetime.datetime.utcnow()
    #                 )
    #                 unlock_embed.set_author(
    ...
    #                     await self.bot.wait_for("message", check=lambda m: m.author.id == 270904126974590976 and ("you're not popular enough and didn't get enough people to rob the bank" in m.content or "for an unsuccessful robbery" in m.content or "Amazing job everybody, we racked up a total of" in m.content), timeout=220)
    #                     await ctx.heist_channel.edit(sync_permissions=True)
    #                     await ctx.send(embed=lock_embed)
