...
class timer(commands.Cog,name= "Giveaway Utils" ,description="Make a giveaway or setup a timer"):
	...
	# 	if webhook is None:
	# 		webhook = await ctx.channel.create_webhook(name=self.bot.user.name,reason = "For Timer Creation", avatar = await self.bot.user.avatar_url.read())
	# 	webhook = DiscordWebhook(url=webhook.url,username = ctx.author.name,avatar_url=f'{ctx.author.avatar_url}')#ctx.author.avatar_url).split("?")[0])
